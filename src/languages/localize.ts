// REF: https://github.com/dermotduffy/frigate-hass-card/blob/main/src/localize/localize.ts
...
export const loadLanguages = async (hass: HomeAssistant): Promise<void> => {
  ...
  if (lang === "cs") {
    languages[lang] = await import("./languages/cs.json");
  }
...
export function localize(string: string, search = "", replace = ""): string {
  ...
  if (!translated) {
    translated = string
      .split(".")
      .reduce((o, i) => o[i], languages[DEFAULT_LANG]);
  }
